...
class StructureEnsemble(Feature):
    """
    Structure Feature
    ...
    def set_scores(self, scoring_function=Functions.analyze_structure): 
        scoring_function(self.sequence, self.structurefile, ensemble=True)
...
class StructureEnsembleAccessibility(StructureEnsemble):
    """
    Manipulate the structure ensemble bases accessibilities
    """
    def __init__(self, structureObject, label = ""):
        StructureEnsemble.__init__(self,structureObject)
        self.label = self.label + label
        self.set_scores()
        self.set_level()

    def set_scores(self, scoring_function=Functions.analyze_structure_accessibility):    
